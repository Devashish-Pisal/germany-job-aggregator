...
class IndeedScraper:
    ...
    def run_scraper(self):
        ...
        logger.info(f"[Indeed Scraper] Indeed found total {len(self.all_query_matched_job_urls)} query matching job urls")
    ...
    def extract_job_urls(self, query_url_list:list[str]) -> set[str]:
        ...
        with sync_playwright() as p:
            browser = p.chromium.launch_persistent_context(
                ...
                user_data_dir=scraper_common_config["scraping_browser_profile_path"], # indeed is blocking requests, browser profile might help
                channel="chrome",
            )
            ...
            try:
                for url in query_url_list:
                    ...
                    is_reject_cookies_visible = page.locator("button[id='onetrust-reject-all-handler']").is_visible()
                    if is_reject_cookies_visible:
                ...
                logger.warning(f"[Indeed Scrapper] Caught exception {e} for url {url}")
        return job_urls
